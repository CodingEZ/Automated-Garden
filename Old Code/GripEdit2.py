...
class Grip:
    ...
    def __init__(self):
        ...
        self.__hsv_threshold_hue = [45.323743322770376, 180.0]
        self.__hsv_threshold_saturation = [0.0, 255.0]
        self.__hsv_threshold_value = [0.0, 247.74743473977358]
        ...
        self.__normalize_input = self.distance_transform_output
...
processClass = Grip()
img = cv2.imread('Leaf.jpg', 1)
processClass.process(img)
cv2.imshow('window1', processClass.normalize_output)

while True:
    k = cv2.waitKey(0)  # milliseconds can be argument
    if k == 27:         # wait for ESC key to exit
        cv2.destroyAllWindows()
        break
    elif k == ord('s'): # wait for 's' key to save and exit
        cv2.imwrite('Edited Leaf.jpg', processClass.normalize_output)
        cv2.destroyAllWindows()
        break
